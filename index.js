...
class Parsing {
  ...
  constructor(filepath) {
    if (fs.existsSync(path.join(filepath, manifestFile)))
      this.lines = fs.readFileSync(path.join(filepath, manifestFile), 'utf-8')
      .split('\n');
    else {
      ...
      if (this._checkJSON(str)) { //如果语句中含有json
        if (this._checkChoice(str)) { // c2.wav {'c5.wav': ['左'], 'c6.wav': ['右']} 01.wav
          ...
          if (this._selected(this.result.flag)) {
            //进入choice
            this.choice.init(this.result.choice, this.result.default)
              .then(msg => {
                this.user.select.push(msg.selected);
                this.user.abandon.concat(msg.unselected);
                this.play.voice(msg.selected)
                  .then(msg => {
                    // console.log("ok");
                    this.getNext();
                  });
          ...
          if (this._selected(this.result.flag)) {
            //进入choice
            this.choice.init(this.result.choice)
              .then(msg => {
                this.user.select.push(msg.selected);
                this.user.abandon.concat(msg.unselected);
                this.play.voice(msg.selected)
        ...
        if (this._checkIfSingle(str)) { //c9.wav end1.wav
          ...
          if (this._selected(this.result.flag)) {
            ...
            this.play.voice(this.result.sound)
              .then(msg => {
                this.getNext();
              });
          } else {
